...
logLik2 <- function(object, ...){
  ...
  K. <- KFS(object$model)
  ll2[4] <- sqrt(mean(K.$v^2, na.rm=TRUE))
#
  ll2
}
```
...
mdls <- c('EWMA1', 'EWMA2', 'EWMA2pres1', 'EWMA2pres2', 
          ...
          'uEWMA1', 'uEWMA1a', 'uEWMA1b', 'uEWMA1pres3', 
          'uEWMA1war0', 'uEWMA1war1', 'uEWMA1p3w0', 
          'Okun', 'OkunPres3', 'OkunWar0', 'OkunP3w0')
n.mdls <- length(mdls)
...
```{r}
logLiks[1, 1:4] <- logLik2(EWMA1fit)
logLiks[1, 1:4]
```
...
```{r}
logLiks[2,  1:4] <- logLik2(EWMA2fit)
logLiks[1:2,1:4]
```
...
logLiks[3,  1:4] <- logLik2(EWMA2pres1fit)
logLiks[1:3,1:4]
```
...
logLiks[4,  1:4] <- logLik2(EWMA2pres2fit)
logLiks[1:4,1:4]
```
...
logLiks[5,  1:4] <- logLik2(EWMA2pres3fit)
logLiks[1:5,1:4]
```
...
logLiks[6,  1:4] <- logLik2(EWMA2war0fit)
logLiks[1:6,1:4]
```
...
logLiks[7,  1:4] <- logLik2(EWMA2war1fit)
logLiks[1:7,1:4]
```
...
logLiks[8,  1:4] <- logLik2(EWMA2war1fit)
logLiks[1:8,1:4]
```
...
```{r}
logLiks[9, c(1:3, 5)] <- logLik2(uEWMA1fit)
logLiks[1:9,]
```
...
logLiks[10,c(1:3, 5)] <- logLik2(uEWMA1aFit)
logLiks[1:10, ]
```
...
logLiks[11,c(1:3, 5)] <- logLik2(uEWMA1bFit)
logLiks[1:11, ]
```
...
uEWMA1pres3fmla <- qlogis(unemployment) ~ 
  SSMtrend(2, Q = list(level=matrix(NA), pres3=EWMA2pres1Q.slope), 
...
uEWMA1pres3fit <- fitSSM(uEWMA1pres3mdl, inits=uEWMA1pres3init, 
                        updatefn=uEWMA1pres3updt) 
logLiks[12, c(1:3, 5)] <- logLik2(uEWMA1pres3fit)
logLiks[1:12,]
```
...
uEWMA1war0fit <- fitSSM(uEWMA1war0mdl, inits=uEWMA1bInit, 
                        uEWMA1war0updt)
logLiks[13, c(1:3, 5)] <- logLik2(uEWMA1war0fit)
logLiks[1:13,]
...
Quite significant.  Let's try a random slope similar to EWMA2war1:    
...
logLiks[14, c(1:3, 5)] <- logLik2(uEWMA1war1fit)
logLiks[1:14,]
```
...
uEWMA1p3w0fit <- fitSSM(uEWMA1p3w0mdl, inits=uEWMA1p3w0init, 
                        uEWMA1p3w0updt)
logLiks[15, c(1:3, 5)] <- logLik2(uEWMA1p3w0fit)
logLiks[13:15, ] 
```
...
Quite likely, the "pres3" and "war0" effects are [confounded](https://en.wikipedia.org/wiki/Confounding) and not independently estimable.  We won't take the time to check that here.  

Instead, we will proceed with to model log(realGDPperCapita) and qlogis(unemployment) with separate "level" parameters for both, looking for a correlation between the changes in the levels, suggested by [Okun's law](https://en.wikipedia.org/wiki/Okun's_law).  In so doing, we will consider using "pres3" with "GDP.lvl" and "war0" with "unemp.lvl", but not "pres3" with "unemp.lvl" nor "war0" with "GDP.lvl".   

# 16.  Modeling GDP and unemployment with Okun's law

[Okun's law](https://en.wikipedia.org/wiki/Okun's_law) suggests that changes in unemployment and GDP are negatively correlated.  `r Citet(bib2, "Horowitz2011")` reported different regression coefficients between the two in different countries.  The [regression coefficient is the correlation coefficient times the ratio of the standard deviations](http://people.duke.edu/~rnau/mathreg.htm).  Thus, the differences between countries in the regression coefficient could be due to differences in either this correlation or either of the standard deviation of changes in GDP or unemployment.  
...
We will look for correlations between all three of these components.  The correlation in the state transition matrix, Q, between GDP.lvl and unemp.lvl seems most closely related to Okun's law, although the correlation between GDP.slope and unemp.lvl also seems relevant to the study of the relationship between unemployment end economic growth, the subject of Okun's Law.  In this modeling, we will assume zero correlation between changed in GDP.lvl and GDP.slope.  
