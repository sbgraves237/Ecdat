...
plotPresWars <- function(x, Data, ..., 
  ...
  for(i in 2:nWars){
    ...
    abline(v=yrs, lty='dotted', col=col.war, lwd=lwd.war)
    yr. <- mean(yrs)
    wy <- (y2.5 + yWar[i-1]*dy)
...
The series obviously does not move as much priot to 1890, because data were not collected then, and the numbers we have were rough estimates for longer periods.  

The period since 1890 with more credible data suggests a war effect with unemployment falling during the Spanish-American-Philippine war, during World Wars I and II, and during Korea, though not Vietnam.  Battle deaths per million population was lower during Vietnam than during Korea and World Wars I and II but not during the Spanish-American-Philippine war.  

Let's check normality of unemployment:  

```{r,fig.cap = "Normality of unemployment"}
qqnorm(GDP$unemployment, datax=TRUE, 
       ylab='unemployment', xlab='standard normal quantiles') 
qqnorm(GDP$unemployment, datax=TRUE, log='x', 
       ylab='unemployment', xlab='standard normal quantiles') 

goodUnempData <- with(GDP, 1889<Year)
qqnorm(GDP$unemployment[goodUnempData], datax=TRUE, 
     ylab='unemployment', xlab='standard normal quantiles') 
qqnorm(GDP$unemployment[goodUnempData], datax=TRUE, log='x', 
       ylab='unemployment', xlab='standard normal quantiles') 
```

Obviously, the numbers are more nearly log-normal than normal.

However, since 0 < unemployment < 1, a better transform might be a symmetrical inverse cumulative distribution function.  Let's first try [qlogis](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/qlogis.html), because it matches the logit transform (log(p/(1-p)), which is closely tied to the log transform and is a primary standard for this kind of work:  

```{r,fig.cap = "Normality of qlogis(unemployment)"}
str(qq.logitUnemp <- qqnorm(qlogis(GDP$unemployment[goodUnempData]),
                        plot=FALSE, datax=TRUE))
plot(qq.logitUnemp, axes=FALSE, 
       xlab='unemployment', ylab='standard normal quantiles') 
axis(2)
unempScale <- c(.02, .05, .1, .2)
axis(1, qlogis(unempScale), unempScale)
```

An alternative standard transformation for proportions is [probits](https://en.wikipedia.org/wiki/Probit), available in R via the [qnorm](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/qnorm.html) function.  Let's try that:  

```{r,fig.cap = "Normality of qnorm(unemployment)"}
str(qq.probitUnemp <- qqnorm(qnorm(GDP$unemployment[goodUnempData]),
                        plot=FALSE, datax=TRUE))
plot(qq.probitUnemp, axes=FALSE, 
       xlab='unemployment', ylab='standard normal quantiles') 
axis(2)
unempScale <- c(.02, .05, .1, .2)
axis(1, qnorm(unempScale), unempScale)
abline(h=0.5)
```

The image using log, qlogis and qnorm differ slightly from one another.  With the linear as well as these three transformations, the plot looks plausibly like two or three straight lines with different slopes plus three points at the low end that may or may not be from another distribution.  This is the pattern of a mixture of distributions with the top 15 or 30 percent of the distribution having both a higher mean and greater volatility and with the last three years of World War II (1943-1945) as possible low outliers.  

The Wikipedia article on [normality test](https://en.wikipedia.org/wiki/Normality_test) (accessed 2016-08-09) said, "A 2011 study concludes that Shapiro–Wilk has the best power for a given significance" compared with three other tests for normality.  This is available in the [shapiro.test](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/formula.html) function:  

```{r Shapiro}
t(cbind(
shapiro.test(GDP$unemployment[goodUnempData])[1:2], 
shapiro.test(log(GDP$unemployment[goodUnempData]))[1:2], 
shapiro.test(qlogis(GDP$unemployment[goodUnempData]))[1:2], 
shapiro.test(qnorm(GDP$unemployment[goodUnempData]))[1:2]))
```

The p-values suggest that unemployment and these three transforms are not independent, normally distributed observations.  It should be obvious that these observations are not independent:  They are time series. Which of these four scales gives us the most normal additive increments?  

```{r dShapiro}
t(cbind(
shapiro.test(diff(GDP$unemployment[goodUnempData]))[1:2], 
shapiro.test(diff(log(GDP$unemployment[goodUnempData])))[1:2],
shapiro.test(diff(qlogis(
  GDP$unemployment[goodUnempData])))[1:2], 
shapiro.test(diff(qnorm(
  GDP$unemployment[goodUnempData])))[1:2]))
```

None of them seem independent normal, but the probit scale seems closer than the others.  We'll use that.  Because of the [central limit theorem](https://en.wikipedia.org/wiki/Central_limit_theorem), the qnorm transformation has the additional advantage of an interpretation as a latent variable that's an accumulation of many small additive adjustments.  

USGDPpresidents includes a variable, "unempSource" indicating the source of the numbers used here.  This is a factor, and we can order a count of observations by its levels:  

```{r unempSource}
(unempS <- table(GDP$unempSource))
```

The help page for USGDPpresidents says that Lebergott is the source for the data until 1890, and Romer's data are used through 1930.  The current standard source, the US Bureau of Labor Statistics (BLS), has data starting in 1940.  

Economists also claim that low unemployment tends increase [inflation](https://en.wikipedia.org/wiki/Inflation), a relationship called the [Phillips curve](https://en.wikipedia.org/wiki/Phillips_curve).  Inflation is widely calculated from changes in a price index, usually the consumer price index.  

We have the CPI in the GDP data.frame, so let's plot it:  

```{r,fig.cap = "Consumer Price Index"}
CPI <- ts(GDP$CPI, 1790)
GDP$CPI <- CPI
plotPresWars(CPI, GDP, log='y', yHoover=1.8, yFDR=2.5, 
  yWar=seq(3, by=-.03, len=9))
``` 

The CPI seemed to increase with some wars but not others.  Let's compute inflation as the first differences in log(CPI):

```{r,fig.cap = "inflation"}
str(inflation <- diff(log(CPI)))
plotPresWars(inflation, GDP, yFDR=1, 
    yWar=seq(-.6, by=0, len=9))
abline(h=0)  
```

We see infation associated with every war in US history [with "war" involving at least 10 battle deaths per million population per year, as discussed in help("USGDPpresidents")], except for the Spanish-American-Philippine war, and that was hardly a war as measured by battle deaths per million population per year. We also note that the inflation associated with World War II was less than the inflation associated with previous wars even though the battle deaths were greater than other wars except for the Civil War.  It should not be difficult to extend the present analysis to model inflation and the [Phillips curve, even allowing the relationship between unemployment and inflation to evolve over time, as indicated by the research on that](https://en.wikipedia.org/wiki/Phillips_curve). However, we will not pursue that further here.  A sequal of the present work for an economics journal should probably include inflation.  
...
With univariate observations on a univariate state, the result is almost a traditional [exponentially weighted moving average (EWMA)](https://en.wikipedia.org/wiki/Exponential_smoothing), except that the weight on the last observation is adjusted with each observation consistent with the Bayesian sequential updating cycle outlined above.  In this case, this weight converges to an asymptote determined by the ratio of the migration and observation error variances `r AutoCite(bib2, "Graves02")`. 
...
4 | GDP/cap | EWMA2pres2: EWMA2 estimating mean slope and no continuity between presidents 
...
8 | GDP/cap | EWMA2war2: EWMA2 with variance effect for war
 | 
9 | unemp | uEWMA1: Bayesian EWMA 
10 | unemp | uEWMA1a: Bayesian EWMA with higher error variance for older data 
11 | unemp | uEWMA1b: Bayesian EWMA with higher error variance prior to 1890  
12 | unemp | uEWMA1pres3: EWMA with a slope adjustment for each president 
13 | unemp | uEWMA1war0: EWMA with slope adjustments for war 
14 | unemp | uEWMA1war1: EWMA with random slope adjustments for war 
15 | unemp | uEWMA1p3w0: EWMA with slope adjustments for presidents and war 
 | 
16 | GDP/cap & unemp | Okun: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) 
17 | GDP/cap & unemp | Okun1: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) assuming perfect correlation between increments in GDP.lvl and GDP.slope 
18 | GDP/cap & unemp | Okun1.1: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) as a univariate problem
19 | GDP/cap & unemp | OkunPres3:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a president effect
20 | GDP/cap & unemp | OkunWar0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a war effect 
21 | GDP/cap & unemp | OkunP3w0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) modeling both president and war 
...
11 | unemp | uEWMA1b: Bayesian EWMA with higher error variance prior to 1890 | 1:level | H | 0 | 1 | 3:Q, H1..2  
12 | unemp | uEWMA1pres3: EWMA with a slope adjustment for each president | 2:level, pres3 | T, Q, H | diag(c(0, NA)) | diag(c(1, 0)) | 4:Q.lvl, Q.pres3, H1..2
13 | unemp | uEWMA1war0: EWMA with fixed slope adjustments for war | 3:level, war, deaths | T, H | 0                 | diag(3)          | 3:Q.lvl, H1..2 
14 | unemp | uEWMA1war1: EWMA with radom slope adjustments for war            | 3:level, war, deaths        | T, Q, H | diag(c(0, Q.war))      | diag(3)             | 6:Q.lvl, Q.war, Q.deaths, Q.warDeaths, H1..2 
15 | unemp | uEWMA1p3w0: EWMA with slope adjustments for presidents and war | 4:level, pres3, war, deaths | T, Q, H | diag(c(0, 1, 0, 0)) | diag(c(1, 0, 1, 1) | 4:Q.lvl, Q.pres3, H1..2 
 | | | | | | | | | 
16 | GDP/cap & unemp | Okun: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) | 3:GDP.lvl, unemp.lvl, GDP.slope | H | 0 | diag(3) | 8:Q.pl, Q.ul, Q.ps, Q.pl.ul, Q.ul.ps, Hp, Hu1, Hu2 
17 | GDP/cap & unemp | Okun1: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) assuming perfect correlation between increments in GDP.lvl and GDP.slope | 3:GDP.lvl, unemp.lvl, GDP.slope | H | 0 | diag(3) | 6:Q.pl, Q.ul, Q.ps, Hp, Hu1, Hu2 
18 | GDP/cap & unemp | Okun1.1: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) as a univariate problem | 3:GDP.lvl, unemp.lvl, GDP.slope | H | 0 | diag(3) | 6:Q.pl, Q.ul, Q.ps, Hp, Hu1, Hu2 
19 | GDP/cap & unemp | OkunPres3:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a president effect | 4:GDP.lvl, unemp.lvl, GDP.slope, GDP.pres3 | T, Q, H | diag(c(0, 0, 0, NA)) | diag(c(1, 1, 1, 0)) | 7:Q.pl, Q.ul, Q.ps, Q.pres3, Hp, Hu1, Hu2 
20 | GDP/cap & unemp | OkunWar0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a war effect | 5:GDP.lvl, unemp.lvl, GDP.slope, unemp.war, unemp.deaths | T, H | 0 | diag(5) | 6:Q.pl, Q.ul, Q.ps, Hp, Hu1, Hu2 
21 | GDP/cap & unemp | OkunP3w0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) modeling both president and war | 6:GDP.lvl, unemp.lvl, GDP.slope, GDP.pres3, unemp.war, unemp.deaths | T, Q, H | diag(c(0, 0, 0, NA, 0, 0)) | diag(c(1, 1, 1, 0, 1, 1)) | 8:Q.pl, Q.ul, Q.ps, Q.pres3, Hp, Hu1, Hu2
...
logLik2 <- function(object, ...){
  ...
  names(ll2) <- c('pars', 'diffuse', 'marginal', 
        'rmse.lnUSD', 'rmse.lgtUnemp')
  ll2 <- data.frame(pars=NA, diffuse=NA, 
      marginal=NA, rmse.lnUSD=NA, rmse.lgtUnemp=NA)
  ...
  if(length(Ks)<2){
    ...
    if((!is.null(Ynames)) && (Ynames=='logitUnemp')){
      ll2[5] <- Ks
    } else ll2[4] <- Ks
  } else ll2[4:5] <- Ks
#
...
mdls <- c('EWMA1', 'EWMA2', 'EWMA2pres1', 'EWMA2pres2', 
          ...
          'EWMA2war2', 'uEWMA1', 'uEWMA1a', 'uEWMA1b',
          'uEWMA1pres3', 'uEWMA1war0', 'uEWMA1war1',
          'uEWMA1p3w0', 'Okun', 'Okun1', 'Okun1.1', 
          'OkunPres3', 'OkunWar0', 'OkunP3w0')
n.mdls <- length(mdls)
logLiks <- data.frame(model=mdls, npars=rep(NA, n.mdls), 
      diffuse=rep(NA, n.mdls), marginal=rep(NA, n.mdls), 
      rmse.lnUSD=rep(NA, n.mdls), 
      rmse.lgtUnemp=rep(NA, n.mdls), row.names=1:n.mdls, 
...
With time series, it's good to look at the [autocorrelation function](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/acf.html) of residuals:  
...
The migration standard deviation for the level is only slightly smaller that for the EWMA1 above.  The migration standard deviation for the slope and measurement error seem much smaller than we might have expected.  It would be interesting to compute a 95 percent confidence interval on the standard deviation of the slope migration;  we will not attempt that here.  (The preferred approach, as previously indicated, would be Monte Carlo.)  
...
logLiks[3, -1] <- logLik2(EWMA2pres1fit)
logLiks[1:3,1:5]
```
...
logLiks[4, -1] <- logLik2(EWMA2pres2fit)
logLiks[1:4,1:5]
```
...
```{r,fig.cap = "Estimated slope of GDP, decoupling the presidents"}
...
This says that the average growth rate is 1.6 percent per year, the standard deviations of the migration for level and slope are 0.038, respectively, and the standard deviation of the observation error is tiny, 0.00004.  
...
logLiks[5, -1] <- logLik2(EWMA2pres3fit)
logLiks[1:5,1:5]
```
...
<center> 
[formula](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/formula.html) -> [SSModel](http://rpackages.ianhowson.com/cran/KFAS/man/SSModel.html) -> [(time varying state transition matrix, T)](http://rpackages.ianhowson.com/cran/KFAS/man/KFAS.html) 
...
logLiks[7, -1] <- logLik2(EWMA2war1fit)
logLiks[1:7,1:5]
```
...
The simplest model for unemployment is just a standard Bayesian EWMA. This is not adequate, because the older data are clearly not as accruately measured.  

However, before considering that. let's start with the simplest model first:  
...
uEWMA1H <- array(NA, c(1, 1, N), 
  dimnames=list('logitUnemp', 'logitUnemp', GDP$Year))
...
```{r}
uEWMA1chisq <- 2*apply(logLiks[9:10, 2:3], 2, diff)
pchisq(uEWMA1chisq, 3, lower=FALSE)
```

Just barely statistically signicant at the traditional 0.05 level:  one chance in 30 of getting a result at least this extreme by chance alone.  

Did this help with the residuals?  

```{r,fig.cap = "Autocorrelation Function of Standardized Residuals"} 
uEWMA1a.KFS <- KFS(uEWMA1aFit$model)
...
No obvious improvement over uEWMA1, but the likelihood ratio chi-square suggests it's still worth keeping.  
...
This estimated a much smaller noise for the most recent data (8.5e-5), with the noise ballooning for the nineteenth-century data, as we expected.  To confirm this, let's plot H:  

```{r,fig.cap = "Observation error variance"}
uEWMA1H. <- ts(as.numeric(uEWMA1aFit$model$H), 
               GDP$Year)

plotPresWars(uEWMA1H., GDP, 
             main="Observation error variance",
             ylab='variance in logits', las=1, log='y', 
             xlab='Year', yHoover=-.5, yFDR=0.25, 
             yWar=seq(-0.5, by=-.3, len=9))
```

Conclusion:  There are really only two differences, not four, in the observation error variance.  

Before continuing, let's delete the two extra parameters to simplify the estimation in looking for president and war effects 

# 11.  Modeling unemployment with higher observation variance for data prior to 1890 

For this, we need to modify the update function to reduce the number of levels for observation variance to 2;  we can reuse uEWMA1aMdl:  

```{r}
uEWMA1bInit <- uEWMA1aInit[1:3]
uEWMA1bUpdt <- function(pars=uEWMA1bInit, model=uEWMA1aMdl, 
                           ...){
  vars <- exp(pars)
  Q <- model$Q
  Q[1,1,] <- vars[1]
...
#
  Yrs <- time(uEWMA1mdl$y)
  H <- (vars[3]+(Yrs<1890)*vars[2])
  model$H[] <- H
  model
} 

uEWMA1bFit <- fitSSM(uEWMA1aMdl, inits=uEWMA1bInit, 
                     updatefn=uEWMA1bUpdt)

logLiks[11, -1] <- logLik2(uEWMA1bFit)
logLiks[1:11, ]
```

Good:  Same numbers as with uEWMA1a.  

The initial plot of unemployment suggested effects due to both presidents and war.  Let's now look for an effect similar to EWMA2pres3.  

# 12. Modeling unemployment with a slope adjustment for each president
...
We now have 4 parameters to estimate:  lnQ.lvl, lnQ.pres3, and lnH1..2.  We need starting values for the parameters to be estimated. As before, we select starting values based on the preceding fit:  

```{r}
uEWMA1bFit$optim.out$par
uEWMA1pres3init <- rep(uEWMA1bFit$optim.out$par, c(2, 1, 1))
...
logLiks[12, -1] <- logLik2(uEWMA1pres3fit)
logLiks[1:12,]
```

The increase in the log(likelihood) is just under 0.4, so the likelihood ratio chi-square is under 0.8.  Clearly that can't be statistically signficant.  

What about a war effect?  

# 13. Modeling unemployment with a fixed slope adjustment for war

The plots above suggest that war reduce unemployment.  Let's look for a fixed effect with a 3-dimensional state vector and a transition matrix as when we looked for a president effect of war:  
...
For starting values, let's look at the the parameter estimates for uEWMA1b:  

```{r}
uEWMA1bFit$optim.out$par
```

We'll use the first two but the third looks too small for a starting value for the logarithm of a variance.  Let's replace the it by the second:  

```{r}
uEWMA1war0init <- rep(uEWMA1bFit$optim.out$par[1:2], c(1, 2))
names(uEWMA1war0init) <- names(uEWMA1bFit$optim.out$par)
uEWMA1war0init
```
...
uEWMA1war0fit <- fitSSM(uEWMA1war0mdl, inits=uEWMA1bInit, 
                        uEWMA1war0updt)
logLiks[13, -1] <- logLik2(uEWMA1war0fit)
logLiks[1:13,]

(uEWMA1war0Chisq <- 2*apply(logLiks[c(11, 13), 2:3], 2, diff))
```

If we count the two values of a1 estimated, double this should be a chi-square 2:  

```{r}
pchisq(uEWMA1war0Chisq[2], 2, lower=FALSE)
```

Quite significant.  This seems to be a better model, even if rmse.lgtUnemp is larger.  
...
```{r}
uEWMA1war1init <- c(lnQ=-2.66, lnQ.war=-1, lnQ.deaths=-1, 
    lnQ.warDeaths=0, lnH1=-4, lnH2=-4)
uEWMA1war1init
...
# 
  Yrs <- time(uEWMA1mdl$y)
  H <- (exp(pars[5])+(Yrs<1890)*exp(pars[6]))
  model$H[] <- H 
  model
} 

uEWMA1war1fit <- fitSSM(uEWMA1war1mdl, inits=uEWMA1war1init, 
                        updatefn=uEWMA1war1updt)
logLiks[14, -1] <- logLik2(uEWMA1war1fit)
logLiks[1:14,]
```

This is worse than for uEWMA1war0 or even uEWMA1b.    
...
OkunDat <- cbind(lnGDPperCap=log(GDP$realGDPperCapita), 
           logitUnemp=qnorm(GDP$unemployment))
...
For starting values, let's use EWMA2 and uEWMA1b, because the current model combines those two models allowing for correlations in the migration variance:  
...
OkunInit[c(2, 7:8)] <- uEWMA1bFit$optim.out$par
OkunInit
...
Unfortunately, "Computation of marginal likelihood failed", and the diffuse likelihood for this case cannot be compared with any of the previous models.  The rmse.lnUSD was better than EWMA1 but slightly worse than EWMA2:  0.0429 vs. 0.0456 and 0.0427.  However, the rmse.lgtUnemp was substantially lower than the best previous model:  0.268 vs. 0.283 for uEWMA1b and 0.284 for uEWMA1war0.  
...
Again, "Computation of marginal likelihood failed".  The diffuse likelihood got substantially worse, but it's not clear if we can interpret that in any meaningful way.  However, both of the root mean square error numbers, rmse.lnUSD and   rmse.lgtUnemp, got a little better.  
...
Before we proceed, we must modify logLik2 so it computes correctely rmse.lnUSD and rmse.lgtUnemp:  
...
The marginal log(likelihood) here should be at least as large as the sum of the log(likelihoods) from EWMA2 and uEWMA1b:  
