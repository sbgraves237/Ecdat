...
`r Citet(bib2, "McChesney2014")` said, "In earlier eras it had been assumed that there was an economic 'guns and butter' tradeoff, and that military spending had to occur at the expense of other sectors of the economy.  However, one of the lessons of the economic expansion in Nazi Germany, followed by the experience of the United States itself in arming for the Second World War, was that big increases in military spending could act as huge stimulants to the economy." `r Citet(bib2, "Pollin2009")` claim their research indicates that tax cuts for household consumption, clean energy, health care and education all generate more jobs per dollar than military spending: 

```{r,fig.cap = "Job creation in the U.S. through $1 billion in spending"}
Pollin.jobs <- c(Military=11600, 
    "Household consumption"=14800, "Clean energy"=17100, 
    "Health care"=19600, Education=29100)
barplot(Pollin.jobs, horiz=TRUE, 
        axisnames=FALSE, col='white')
text(2000, seq(.65, by=1.2, len=5), names(Pollin.jobs), 
     cex=1.2, adj=0)
```


As previously mentioned, the primary purpose of this work is to demostrate the use of the [KFAS](


) package for addressing questions like those considered here.  Better answers would require simultaneous modeling of unemployment 

with 

inflation, as [Milton Friedman](


) famously wrote, and possibly including other factors such as education (
...
Whatever the specific cause, the growth of the U.S. economy under FDR before the war was not matched by that of any other president (not counting the negative record of [Herbert Hoover](https://en.wikipedia.org/wiki/Herbert_Hoover)).  This document describes alternative attempts to quantify the economic impact of wars separate from that of the administration ("president").  This includes a detailed discussion and comparison of alternative models of economic growth and how [KFAS](https://rweb.crmda.ku.edu/cran/web/packages/KFAS/KFAS.pdf) was used to fit them.  The conclusions regarding FDR vs. WW II must be considered tentative, pending further research using other data.
...
In the following, we shall consider eight different though related models:  
...
7 | EWMA2war1: EWMA2 with random effect of war | 4:level, slope, war0, war1 | Q, T | 6:lnQ.lvl, lnQ.slope, lnQ.war00, lnQ.war11, lnQ.war01, lnH
 | | | |
8 | EWMA2presWar: EWMA2pres4 plus war ??? | 5:level, slope, Pres, war0, war1 | Q, T | ??:lnQ.lvl, lnQ.slope, lnQ.Pres, lnH, ???
...
```{r}
EWMA1.v <- (log(GDP$realGDPperCapita)-EWMA1pred)         
```
...
However, as discussed above, the assumptions for Wilks's theorem do not apply here.  The standard recommendation would be to compute a p-value for this using Monte Carlo.  Before going to that trouble, we first try the modification suggested by `r Citet(bib2, "PinBates")`, mentioned above:  
...
The predictions are better than the standard EWMA, but the prediction limits are too tight:  Most of Hoover's presidency is below the lower prediction limit.  FDR started below the lower limit and ended above the upper limit.  However, the economy in 1948 at the end of the post-war recession was moderately close to what would have been predicted for it in 1928 before the Great Depression;  this is a clear improvement over the standard EWMA discussed above.
...
This is 0.0427, matching the previous model.  We built this model hoping for a reduction.  However, since the log(likelihood) is not larger, we could not expect this to be smaller.  
...
Formally at least, we have the same parameters to estimate as with EWMA2pres1.  However, we can set $P1_{2,2}$ to match $Q_{2,2,t}$ for the last year of each presidency;  note that P1 = "num [1:2, 1:2, 1] 0 0 0 0" in "str(EWMA2mdl)" above.  
...
where $\gamma_t$ = 0 for the last year of a presidency, and 1 otherwise, as with model EWMA2pres2, above.  
...
This is the best [logLik](http://rpackages.ianhowson.com/cran/KFAS/man/logLik.SSModel.html) so far.  However, the increase in logLik is slightly less than 2, which makes the [AIC](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/AIC.html) and the BIC worse.  

Let's also check the root mean square one-step ahead prediction error:  
...
This confirms the apparent superiority of this model fit.  
...
In this case, Q is constant.  This suggests we may not have to create a time-varying Q nor an updatefn as we did for models 3-5.  This simplifies the modeling process to the following:  
...
This model has the same 3 parameters to estimate as models 2, 3 and 4 above.  We will use the same initial values:  EWMA2init.

```{r}
(EWMA2war0fit <- fitSSM(EWMA2war0mdl, inits=EWMA2init)) 
logLik(EWMA2war0fit$model)
```

At 370.9859, this looks like a worse fit than EWMA1.  

Let's try [logLik(..., marginal=TRUE, ..)](http://rpackages.ianhowson.com/cran/KFAS/man/logLik.SSModel.html):  

```{r}
logLik(EWMA2war0fit$model,marginal=TRUE)
logLik(EWMA2fit$model,marginal=TRUE)
```

Conclusion:  If we use the marginal likelihood rather than the diffuse likelihood, it says that EWMA2war0 gives a slightly better fit:  394.7362 vs. 394.2869.  In other words, the difference was primarily due to the use of diffuse likelihood, which is different with the addition of another component to the state vector.  
...
logLik(EWMA2war0fit2$model)
logLik(EWMA2war0fit2$model, marginal=TRUE)
```

This gives the same [logLik](http://rpackages.ianhowson.com/cran/KFAS/man/logLik.SSModel.html) as EWMA2, as we might expect.  Let's modify the modeling process to include an updatefn to allow us to estimate P1[3:4, 3:4] different from diag(0, 2):  
...
(EWMA2war0fit4 <- fitSSM(EWMA2war0mdl2, inits=EWMA2war0init, 
                        updatefn=EWMA2war0updt))$optim.out
```
...
(EWMA2war0fit5 <- fitSSM(EWMA2war0mdl2, inits=EWMA2war0init2, 
                        updatefn=EWMA2war0updt))$optim.out
```

As expected.  Let's try something between EWMA2war0init and EWMA2war0init2:  
...
At logLik = 383.8239, this is not as good as EWMA2war0fit4.  We might get something better with different starting values.  
...
```{r}
EWMA2war0.KFS <- KFS(EWMA2war0fit5$model)
plot(EWMA2war0.KFS$alphahat)
apply(EWMA2war0.KFS$alphahat, 2, range)
sqrt(mean(EWMA2war0.KFS$v^2))
```

0.0428

JOUNI:  WHAT AM I DOING WRONG?  WHY CAN'T I GET A BETTER FIT 
FROM THIS MODEL AS FROM THE BASE EWMA2?  
...
(EWMA2war1fit <- fitSSM(EWMA2war1mdl, inits=EWMA2war1init, 
                        updatefn=EWMA2war1updt))$optim.out
```
...
(EWMA2war1fit0 <- fitSSM(EWMA2war1mdl, inits=EWMA2war1init0, 
                        updatefn=EWMA2war1updt))$optim.out
logLik(EWMA2war1fit0$model)
logLik(EWMA2war0fit$model, marginal=TRUE)
logLik(EWMA2war1fit0$model, marginal=TRUE)
logLik(EWMA2fit$model, marginal=TRUE)
```
