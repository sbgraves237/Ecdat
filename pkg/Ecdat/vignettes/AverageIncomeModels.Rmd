...
Let's try again with inits=EWMA2fit\$optim.out\$pars:  

```{r}
(EWMA2war0fit2 <- fitSSM(EWMA2war0mdl, inits=EWMA2init)) 
logLik(EWMA2war0fit2$model)
```

Same nonsense answer.  
...
EWMA2war0fit3 <- fitSSM(EWMA2war0mdl2, inits=EWMA2init)
logLik(EWMA2war0fit3$model)
```

This gives the same [logLik](http://rpackages.ianhowson.com/cran/KFAS/man/logLik.SSModel.html) as EWMA2.  Let's modify the modeling process to include an updatefn to allow us to estimate P1[3:4, 3:4] different from diag(0, 2):  
...
This is worse than EWMA2war0fit, EWMA2war0fit2 and EWMA2war0fit3.  Let's check with starting values designed to give 
the same answer as EWMA2:  
...
JOUNI:  What am I doing wrong?  Thanks, sg
???
