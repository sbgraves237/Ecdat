...
10 | unemp | uEWMA1a: Bayesian EWMA with higher noise variance for older data 
11 | unemp | uEWMA1b: Bayesian EWMA with higher noise variance prior to 1890  
...
16 | GDP/cap & unemp | OkunPres3:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a president effect
17 | GDP/cap & unemp | OkunWar0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a war effect 
18 | GDP/cap & unemp | OkunP3w0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) modeling both president and war 
...
4 | GDP/cap | EWMA2pres2: EWMA2 estimating mean slope and no continuity between presidents | 3:level, slope, a1.slope | Q, T | diag(c(0, NA, 0)) | diag(c(1, 0, 0)) | 4:a1.slope, lnQ.lvl, lnQ.slope, lnH
5 | GDP/cap | EWMA2pres3: EWMA2 with an adjustment for each president | 3:level, slope, pres3 | Q, T | diag(c(0, 0, NA) | diag(c(1, 1, 0)) | 4:lnQ.lvl, lnQ.slope, lnQ.pres3, lnH 
 | | | | | | | |  
6 | GDP/cap | EWMA2war0: EWMA2 with fixed effect of war | 4:level, slope, war, deaths | T | 0 | diag(4) | 3:lnQ.lvl, lnQ.slope, lnH
7 | GDP/cap | EWMA2war1: EWMA2 with random effect of war | 4:level, slope, war, deaths | Q, T | diag(c(0, 0, Q.war))	| diag(c(1, 1, 0, 0)) | 6:lnQ.lvl, lnQ.slope, lnQ.war, lnQ.deaths, lnQ.warDeaths, lnH
...
10 | unemp | uEWMA1a: EWMA with higher noise variance for older data | 1:level | | 0 | 1 | 5:lnQ, lnH1..4 
11 | unemp | uEWMA1b: Bayesian EWMA with higher noise variance prior to 1890 | 1:level | | 0 | 1 | 3:lnQ, lnH1..2  
12 | unemp | uEWMA1pres3: EWMA with a slope adjustment for each president | 2:level, pres3 | Q, T | diag(c(0, NA)) | diag(c(1, 0)) | 4:lnQ.lvl, lnQ.pres3, lnH1..2
13 | unemp | uEWMA1war0: EWMA with slope adjustments for war | 3:level, war, deaths | T | 0 | diag(3) | 5:lnQ.lvl, lnQ.war, lnQ.deaths, lnH1..2 
14 | unemp | uEWMA1p3w0: EWMA with slope adjustments for presidents and war | 4:level, pres3, war, deaths | Q, T | diag(c(0, 1, 0, 0))
| diag(c(1, 0, 1, 1) | 6:lnQ.lvl, lnQ.pres3, lnQ.war, lnQ.deaths, lnH1..2 
 | | | | | | | |  
15 | GDP/cap & unemp | Okun: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law)
16 | GDP/cap & unemp | OkunPres3:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a president effect
17 | GDP/cap & unemp | OkunWar0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a war effect 
...
mdls <- c('EWMA1', 'EWMA2', 'EWMA2pres1', 'EWMA2pres2', 
          ...
          'uEWMA1war0', 'uEWMA1p3w0', 'Okun', 'OkunPres3', 
          'OkunWar0', 'OkunP3w0')
n.mdls <- length(mdls)
...
This doesn't look great.  What happens if we force the noise variance to increase for older data?  

# 10.  Modeling unemployment with higher observation variance for older data 

For this, we can use the same [formula](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/formula.html) as for uEWMA1, but we'll need to make the observation noise variane, H, time varying, and we'll also need a special update function:  

<center> 
[formula](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/formula.html) -> [(make the observation noise variance, H, time varying)](http://rpackages.ianhowson.com/cran/KFAS/man/KFAS.html) -> [SSModel](http://rpackages.ianhowson.com/cran/KFAS/man/SSModel.html) 
...
Let's start by modifying the formula, similar to what we did with EWMA2pres1fmla, above, but for this 2-dimensional state    
...
# 13. Modeling unemployment with a slope adjustment for war
...
For this model, we do not need to create a time-varying Q nor an updatefn.  This simplifies the modeling process to the following:  
...
uEWMA1war0mdl. <- SSModel(uEWMA1war0fmla, GDP, H=matrix(NA))
uEWMA1war0mdl <- fixModelNames(uEWMA1war0mdl., names(a1.3u) )
```
...
```{r}
attr(uEWMA1war0mdl, 'tv') <- c(Z=0L, H=0L, T=1L, R=0L, Q=0L)
```

This model has same 3 parameters to estimate as uEWMA1b:  lnQ.lvl, lnH1, and lnH2. We'll use the same starting values: uEWMA1bInit.
...
The marginal likelihood increased by 1.66.  If we count the two values of a1 estimated, double this should be a chi-square 2:  
...
# 14.  Modeling unemployment with slope adjustments for presidents and war 
