...
Whatever the specific cause, the growth of the U.S. economy under [FDR](https://en.wikipedia.org/wiki/Franklin_D._Roosevelt) before the war signficantly exceeds that of any other president (not counting the negative record of [Herbert Hoover](https://en.wikipedia.org/wiki/Herbert_Hoover)).  This document describes alternative attempts to quantify the economic impact of wars separate from that of the administration ("president").  This includes a detailed discussion and comparison of alternative models of economic growth and how [KFAS](https://rweb.crmda.ku.edu/cran/web/packages/KFAS/KFAS.pdf) was used to fit them.  

The best of the models we fit include a "Presidency" but not a war effect for real GDP per capita.  We found the opposite when considering unemployment:  a war but not an administration effect. However, these conclusions must be considered tentative, pending further research using other data and possibly other ways of modeling the macroeconomic impact of war.  
...
GDP.$Year[chgSgn][1:3]
GDP.[1:9, c('Year', 'realGDPperCapita')]
```
...
Let's look closer at World War II:  

```{r,fig.cap = "Unemployment and World War II"}
plotPresWars(unemployment, GDP, 
             main=paste0('US unemployment'), 
             ylab='unemployment rate', las=1, 
             yHoover=0.07, yFDR=0.25, 
...
The series obviously does not move as much prior to 1890, because data were not collected then, and the numbers we have were rough estimates for longer periods.  
...
For other purposes, let's digress to review the record of unemployment since World War II:  

```{r,fig.cap = "Unemployment since World War II"}
plotPresWars(unemployment, GDP, 
             main=paste0('US unemployment'), 
             ylab='unemployment rate', las=1, 
             yHoover=0.07, yFDR=0.25, 
             yWar=seq(.5, by=-.05, len=9),
             xlim=c(1945, 2015), ylim=c(0.01, .1)) 
abline(v=seq(1945, 2015, by=4))
```

The most consistent declines during this period occurred between 1961 and 1969 and between 1993 and 2001, the presidencies of Kennedy, Johnson, and Clinton.  This is closely followed by 
...
11 | unemp | uEWMA1pres3: EWMA with a slope adjustment for each president 
12 | unemp | uEWMA1war0: EWMA with slope adjustments for war 
13 | unemp | uEWMA1war1: EWMA with random slope adjustments for war
14 | unemp | uEWMA1p3w0: EWMA with slope adjustments for presidents and war 
 | 
15 | GDP/cap & unemp | Okun: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) 
16 | GDP/cap & unemp | Okun1: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) assuming perfect correlation between increments in GDP.lvl and GDP.slope 
17 | GDP/cap & unemp | Okun1.1: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) as a univariate problem
18 | GDP/cap & unemp | OkunPres3:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a president effect
19 | GDP/cap & unemp | OkunWar0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a war effect 
20 | GDP/cap & unemp | OkunP3w0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) modeling both president and war 
...
14 | unemp | uEWMA1p3w0: EWMA with slope adjustments for presidents and war | 4:level, pres3, war, deaths | T, Q, H | diag(c(0, 1, 0, 0)) | diag(c(1, 0, 1, 1) | 6:Q.lvl, Q.pres3, H1..4 
 | | | | | | | | | 
15 | GDP/cap & unemp | Okun: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) | 3:GDP.lvl, unemp.lvl, GDP.slope | H | 0 | diag(3) | 10:Q.pl, Q.ul, Q.ps, Q.pl.ul, Q.ul.ps, Hp, Hu1..4 
16 | GDP/cap & unemp | Okun1: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) assuming perfect correlation between increments in GDP.lvl and GDP.slope | 3:GDP.lvl, unemp.lvl, GDP.slope | H | 0 | diag(3) | 8:Q.pl, Q.ul, Q.ps, Hp, Hu1..4
17 | GDP/cap & unemp | Okun1.1: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) as a univariate problem | 3:GDP.lvl, unemp.lvl, GDP.slope | H | 0 | diag(3) | 8:Q.pl, Q.ul, Q.ps, Hp, Hu1..4
18 | GDP/cap & unemp | OkunPres3:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a president effect | 4:GDP.lvl, unemp.lvl, GDP.slope, GDP.pres3 | T, Q, H | diag(c(0, 0, 0, NA)) | diag(c(1, 1, 1, 0)) | 9:Q.pl, Q.ul, Q.ps, Q.pres3, Hp, Hu1..4
19 | GDP/cap & unemp | OkunWar0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a war effect | 5:GDP.lvl, unemp.lvl, GDP.slope, unemp.war, unemp.deaths | T, H | 0 | diag(5) | 8:Q.pl, Q.ul, Q.ps, Hp, Hu1..4 
20 | GDP/cap & unemp | OkunP3w0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) modeling both president and war | 6:GDP.lvl, unemp.lvl, GDP.slope, GDP.pres3, unemp.war, unemp.deaths | T, Q, H | diag(c(0, 0, 0, NA, 0, 0)) | diag(c(1, 1, 1, 0, 1, 1)) | 10:Q.pl, Q.ul, Q.ps, Q.pres3, Hp, Hu1..4
...
Let's wrap these three goodness-of-fit measures in a fuction with the number of parameters estimated, so we can easily compute a 4-number summary for subsequent model comparison.  For present purposes, the number of parameters estimated (ndf = number of degrees of freedom) is the sum of two terms:  the number of components of the state vector with diffuse priors and the number of parameters estimated using a nonlinear optimizer:  
...
This understates the autocorrelation, because by default, the [acf](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/acf.html) function subtracts off the mean of the series, and the mean should theoretically be zero for residuals.  We get a different picture with "dmean=FALSE":  
...
Modeling is a straightforward generalization of the above model following the same process:
...
```{r EWMA2p1fit}
system.time(EWMA2pres1fit <- fitSSM(EWMA2pres1mdl, 
...
system.time(EWMA2pres1fit2 <- fitSSM(EWMA2pres1mdl,
...
This stopped with a value out of bounds and a negative log(likelihood) slightly above the unconstrained fit:  

  *  Q.slope is negative, even though lower = 0.  
  
  * value = (-386.6286) vs. (-386.9081) for the unconstrained optimization.  

Let's try again, starting with the previous optim:  
...
likfn <- function(pars=EWMA2init2, model=EWMA2pres1mdl, 
    ...
    Model <- do.call(updatefn, args = list(pars, model))
...
```{r nloptr}
library(nloptr)
(try(nloptr(EWMA2init2, likfn,
        model=EWMA2pres1mdl, updatefn=EWMA2pres1updt2)))
(EWMA2pres1fit2c <- nloptr(EWMA2init2, likfn, lb=rep(0, 3), 
        opts=list(algorithm='NLOPT_LN_BOBYQA'), 
        model=EWMA2pres1mdl, updatefn=EWMA2pres1updt2))
(EWMA2pres1fit2d <- nloptr(EWMA2p1init, likfn, lb=rep(0, 3), 
        opts=list(algorithm='NLOPT_LN_BOBYQA'), 
        model=EWMA2pres1mdl, updatefn=EWMA2pres1updt2))

(EWMA2pres1fit2e <- nloptr(EWMA2init2, likfn, lb=rep(0, 3), 
        opts=list(algorithm='NLOPT_LN_NELDERMEAD'), 
        model=EWMA2pres1mdl, updatefn=EWMA2pres1updt2))
(EWMA2pres1fit2f <- nloptr(EWMA2p1init, likfn, lb=rep(0, 3), 
        opts=list(algorithm='NLOPT_LN_NELDERMEAD'), 
        model=EWMA2pres1mdl, updatefn=EWMA2pres1updt2))
```

Conclusion:  We could experiment with more algorithms and starting values, but the best solutions so far match the original solution. 
...
```{r}
(d2.8 <- apply(logLiks[c(2, 8), -c(1, 6)], 2, diff))
...
Quite significant.  This seems to be a better model, even if the unweighted rmse.pbtUnemp is larger.  
...
uEWMA1war1init <- c(lnQ=-2.66, lnQ.war=-1, lnQ.deaths=-1, 
    lnQ.warDeaths=0, lnH1=-4, lnH2=-4, lnH3=-4, lnH4=-4)
uEWMA1war1init
...
# 
  ...
  H <- (exp(pars[8])+(Yrs<1890)*exp(pars[7]) +
      (Yrs<1931)*exp(pars[6])+(Yrs<1890)*exp(pars[4]))
  model$H[] <- H 
  model
} 
...
logLiks[13, -1] <- logLik2(uEWMA1war1fit)
logLiks[1:13,]
```

This is worse than for uEWMA1war0 or even uEWMA1a.    
