...
The series obviously does not move as much priot to 1890, because data were not collected then, and the numbers we have were roughly estimates for longer periods.  
...
However, since 0 < unemployment < 1, a better transform might be a symmetrical inverse cumulative distribution function.  We'll try [qlogis]https://stat.ethz.ch/R-manual/R-devel/library/stats/html/qlogis.html), because it matches the logit transform (log(p/(1-p)), which is closely tied to the log transform and is a primary standard for this kind of work:  
...
The image with [qlogis]https://stat.ethz.ch/R-manual/R-devel/library/stats/html/qlogis.html) differs slighly from the log-normal:  With both, the high unemployment period of the Great Depression appear as high outliers while the low unemployment of the last three years of World War II (1943-1945) appear as low outliers:  Those periods do appear to be different than the rest of this historical period.  A log transform emphasizes the latter while the logit emphasizes the former more.  We'll use the logit, because it makes more sense theoretically.
...
15 | unemp | uEWMA1p3w0: EWMA with slope adjustments for presidents and war 
 | 
16 | GDP/cap & unemp | Okun: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) | 3:GDP.lvl, GDP.slope, unemp.lvl | H | 0 | diag(3) | 8:lnQ.pl, lnQ.ul, lnQ.ps, lnQ.pl.ul, lnQ.ul.ps, lnHp, lnHu1, lnHu2 
17 | GDP/cap & unemp | OkunPres3:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a president effect
18 | GDP/cap & unemp | OkunWar0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a war effect 
19 | GDP/cap & unemp | OkunP3w0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) modeling both president and war 
...
-:|:-----|------------------|:-----|:----:|:---:|:---:|:--------------
1 | GDP/cap | EWMA1: Standard Bayesian EWMA | 1:level | | 0 | 1 | 2:lnQ, lnH
2 | GDP/cap | EWMA2: double EWMA | 2:level, slope | | 0 | diag(2) | 3:lnQ.lvl, lnQ.slope, lnH
3 | GDP/cap | EWMA2pres1: EWMA2 allowing slope to change only between presidents | 2:level, slope | Q | 0 | diag(2) | 3:lnQ.lvl, lnQ.slope, lnH 
4 | GDP/cap | EWMA2pres2: EWMA2 estimating mean slope and no continuity between presidents | 3:level, slope, a1.slope | T, Q | diag(c(0, NA, 0)) | diag(c(1, 0, 0)) | 4:a1.slope, lnQ.lvl, lnQ.slope, lnH
5 | GDP/cap | EWMA2pres3: EWMA2 with an adjustment for each president | 3:level, slope, pres3 | T, Q | diag(c(0, 0, NA) | diag(c(1, 1, 0)) | 4:lnQ.lvl, lnQ.slope, lnQ.pres3, lnH 
 | | | | | | | |  
6 | GDP/cap | EWMA2war0: EWMA2 with fixed effect of war | 4:level, slope, war, deaths | T | 0 | diag(4) | 3:lnQ.lvl, lnQ.slope, lnH
7 | GDP/cap | EWMA2war1: EWMA2 with random effect of war | 4:level, slope, war, deaths | T, Q | diag(c(0, 0, Q.war))	| diag(c(1, 1, 0, 0)) | 6:lnQ.lvl, lnQ.slope, lnQ.war, lnQ.deaths, lnQ.warDeaths, lnH
8 | GDP/cap | EWMA2war2: EWMA2 with variance effect for war |	2:level, slope |	Q |	0 | diag(2) | 5:lnQ.lvl, lnQ.slope, lnQ_war, lnQ_deaths, lnH
 | | | | | | | |  
9 | unemp | uEWMA1: Standard Bayesian EWMA | 1:level | | 0 | 1 | 2:lnQ, lnH
10 | unemp | uEWMA1a: EWMA with higher error variance for older data | 1:level | H | 0 | 1 | 5:lnQ, lnH1..4 
11 | unemp | uEWMA1b: Bayesian EWMA with higher error variance prior to 1890 | 1:level | H | 0 | 1 | 3:lnQ, lnH1..2  
12 | unemp | uEWMA1pres3: EWMA with a slope adjustment for each president | 2:level, pres3 | T, Q, H | diag(c(0, NA)) | diag(c(1, 0)) | 4:lnQ.lvl, lnQ.pres3, lnH1..2
13 | unemp | uEWMA1war0: EWMA with fixed slope adjustments for war | 3:level, war, deaths | T, H | 0                 | diag(3)          | 5:lnQ.lvl, lnQ.war, lnQ.deaths, lnH1..2 
14 | unemp | uEWMA1war1: EWMA with radom slope adjustments for war            | 3:level, war, deaths        | T, Q, H | diag(c(0, Q.war))      | diag(3)             | 6:lnQ.lvl, lnQ.war, lnQ.deaths, lnQ.warDeaths, lnH1..2 
15 | unemp | uEWMA1p3w0: EWMA with slope adjustments for presidents and war | 4:level, pres3, war, deaths | T, Q, H | diag(c(0, 1, 0, 0)) | diag(c(1, 0, 1, 1) | 6:lnQ.lvl, lnQ.pres3, lnQ.war, lnQ.deaths, lnH1..2 
 | | | | | | | | | 
16 | GDP/cap & unemp | Okun: [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) | 3:GDP.lvl, unemp.lvl, GDP.slope | H | 0 | diag(3) | 8:lnQ.pl, lnQ.ul, lnQ.ps, lnQ.pl.ul, lnQ.ul.ps, lnHp, lnHu1, lnHu2 
17 | GDP/cap & unemp | OkunPres3:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a president effect
18 | GDP/cap & unemp | OkunWar0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a war effect 
19 | GDP/cap & unemp | OkunP3w0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) modeling both president and war 
...
logLik2 <- function(object, ...){
  ...
  names(ll2) <- c('pars', 'diffuse', 'marginal', 
        'rmse.logDollars', 'rmse.logitUnemp')
...
mdls <- c('EWMA1', 'EWMA2', 'EWMA2pres1', 'EWMA2pres2', 
          ...
          'uEWMA1p3w0', 'Okun', 'OkunPres3', 'OkunWar0',
          'OkunP3w0')
n.mdls <- length(mdls)
logLiks <- data.frame(npars=rep(NA, n.mdls), 
      diffuse=rep(NA, n.mdls), marginal=rep(NA, n.mdls), 
      rmse.logDollars=rep(NA, n.mdls), 
      rmse.logitUnemp=rep(NA, n.mdls), row.names=mdls)
```
...
uEWMA1fmla <- qlogis(unemployment) ~
...
ylim <- range(qlogis(GDP$unemployment), na.rm=TRUE)
plotPresWars(qlogis(unemployment), GDP, 
             main="EWMA of qlogis(Unemployment)", ylim=ylim, 
             ylab='unemployment rate', las=1, 
             type='n', xlab='Year', axes=FALSE, 
             yHoover=0.07, yFDR=0.25, 
             yWar=seq(.5, by=-.05, len=9))
text(qlogis(GDP$unemployment), 'x', cex=0.5)
matlines(GDP$Year, uEWMA1pred, 
         col=c('blue', 'darkgreen', 'darkgreen'), 
         lty=c(1, 2, 2))
axis(1)
axis(2, qlogis(unempScale), unempScale)
```

This looks plausible.  What about the log(likelihood)?  

```{r}
logLiks[9, ] <- logLik2(uEWMA1fit)
logLiks[1:9,]
```
...
One answer to this question uses a 2-dimensional state vector of level (of qlogis(unemployment)) and a slope term that restarts with each administration similar to the "pres3" term with EWMA2pres3.  For this we use a state transition matrix like the following:  
...
logLiks[12, ] <- logLik2(uEWMA1pres3fit)
logLiks[1:12,]
```
...
```{r}
a1.3u <- c(level=qlogis(unemp1), war=0, deaths=0)
uWarStates <- names(a1.3u)

uEWMA1war0fmla <- qlogis(unemployment) ~ 
...
uEWMA1war0fit <- fitSSM(uEWMA1war0mdl, inits=uEWMA1bInit, 
                        uEWMA1war0updt)
logLiks[13, ] <- logLik2(uEWMA1war0fit)
logLiks[1:13,]
...
Quite significant.  This seems to be a better model, even if rmse.logitUnemp is larger.  
...
where $\omega_t$ = 1 during wars and 0 otherwise as before.  And we need to make the migration variance time varying to match: 
...
logLiks[14, ] <- logLik2(uEWMA1war1fit)
logLiks[1:14,]
```
...
```{r}
a1.4u <- c(level=qlogis(unemp1), pres3=0, war=0, deaths=0)
...
uEWMA1p3w0fit <- fitSSM(uEWMA1p3w0mdl, inits=uEWMA1p3w0init, 
                        uEWMA1p3w0updt)
logLiks[15, ] <- logLik2(uEWMA1p3w0fit)
logLiks[13:15, ] 
```
...
We could estimate this model with KFAS with an entirely univariate response updating alternately between log(realGDPperCapita) and qlogis(unemployment) -- or even mixing annual GDP figures with quarterly unemployment numbers or mixing annual and quarterly data in other ways. 
...
In coding this we will follow the example on pp. 15ff of `r Citet(bib2, "KFASarticle")`, which has a 4-dimensional response:  

<center> 
Respose matrix -> Z, T, Q, a1, P1, P1inf -> [SSMcustom](http://rpackages.ianhowson.com/cran/KFAS/man/SSModel.html) in 
...
OkunDat <- cbind(lnGDPperCap=log(GDP$realGDPperCapita), 
           logitUnemp=qlogis(GDP$unemployment))
...
For starting values, let's look at EWMA2pres3 and uEWMA1war0: 

```{r}
EWMA2pres3fit$optim.out$par
uEWMA1war0fit$optim.out$par

OkunInit <- c(lnQ.pl=-6, lnQ.ul=-2, lnQ.ps=-8, 
  lnQ.pl.ul=0, lnQ.ul.ps=0, lnHp=-8, lnHu1=-4, lnHu2=-8)
...
logLiks[16, ] <- logLik2(OkunFit)  
logLiks[1:16, ]
```  
  
Unfortunately, "Computation of marginal likelihood failed", and the diffuse likelihood for this case cannot be compared with any of the previous models.  The rmse.logDollars was better than EWMA1 but slightly worse than EWMA2:  0.0429 vs. 0.0456 and 0.0427.  However, the rmse.logitUnemp was substantially lower than the best previous model:  0.224 vs. 0.283 for uEWMA1b and 0.284 for uEWMA1war0.  

What about the parameter estimates?  
...
The primary Okun correlation is -0.78.  This is negative as expected and quite strong.    

The secondary correlation between change in unemp.lvl and GDP.slope is positive and still moderately strong at 0.62.  This suggests that a stronger than normal reduction in unemployment will tend to co-occur with a short term increase in GDP combined with a reduction in the longer term rate of economic growth.  

It would be useful to do a Monte Carlo to obtain confidence intervals on these 5 parameters in 5 incarnations:  

  1.  a 5-dimensional vector in the pdLogChol representation. 
  1.  the logarithms of the 3 variance parameters and the logits of the 2 correlations.  
  1.  the 3 variance parameters and 2 correlations.  
  1.  the variances of the untransformed GDP per capita and unemployment rate spaces along with the corresponding correlations.  
  1.  back transforming these results into the space of GDP per capita, its rate of change, and the unemployment rate used in the mainstream literature on Okun's Law, e.g., `r Citet(bib2, "BallOkun")`.  
  
We will attempt none of this here at this time.  

Rather, we will consider modifications of this model to include "pres3" and / or "war0".  

# 17.  Okun's law with a president effect 


| GDP/cap & unemp | OkunPres3:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a president effect

# 18. Okun's law with a war effect 

| GDP/cap & unemp | OkunWar0:  [Okun's law](https://en.wikipedia.org/wiki/Okun's_law) with a war effect 


# 19.  Okun's law considering both presidents and wars 
