...
In the following, we shall consider eight different though related models:  
 ...
 || Model 
-:|:-----------------------------------------------
1 | EWMA1: Standard Bayesian EWMA
2 | EWMA2: double EWMA 
3 | EWMA2pres1: EWMA2 allowing slope to change only between presidents  
4 | EWMA2pres2: EWMA2 estimating mean slope and no continuity between 
5 | EWMA2pres3: EWMA2 with an adjustment for each president 
 | 
6 | EWMA2war0: EWMA2 with fixed effect of war 
7 | EWMA2war1: EWMA2 with random effect of war 
8 | EWMA2war2: EWMA2 with variance effect for war
...
If the model fit well, then the residuals should have zero mean.  The difference between the acfs with demean = TRUE (the default) and FALSE is a result of the error in that assumption.  We would expect this difference to disappear with a double EWMA considered in the next section.  
...
Before moving to that, however, let's plot these residuals vs. Presidents and wars:  
