animate1 <- function(plotList, nFrames=NULL, iFrame=NULL,
...
#    Fns <- sapply(plotList, function(z){
#        fn <- z$fun
#        if(is.null(fn)){
#            NA
#        } else fn
#    })
    ...
    if(length(noFun)>0){
        stop('plotList[', noFun[1], '] must have an element ',
             '"fun";  it does not.')
    }
##
## 3.  par
...
##
## 4.  loop over all function calls in plotList
##
    nFns <- length(Fn.)
    for(j in seq(length=nFns)){
        ...
        if(length(oops1)>0){
            ...
            warning('plotList$', names(Fn.)[j], '$firstFrame[',
        ...
        if(length(oopsL)>0){
            ...
            warning('plotList$', names(Fn.)[j], '$lastFrame[',
